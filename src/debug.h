...
void receiveFeedback(uint8_t addr, uint8_t command) {
  ...
  if (Serial2.available() >= expectedBytes) {
    ...
    // Xử lý dữ liệu dựa trên command
    if (command == 0x33) { // Số xung (int32_t) - BIG ENDIAN
      Serial.println("Reading pulse data (4 bytes)...");
      uint32_t pulse = 0;
      uint8_t data[4];
      for (int i = 0; i < 4; i++) {
        data[i] = Serial2.read();
      ...
      // Tính góc từ pulse (3200 steps/revolution cho MKS SERVO42C)
  ...
  } else if (command == 0x30) { // Góc encoder (carry + value)
    Serial.println("Reading encoder data (6 bytes: carry + value)...");
    
    // Đọc carry (4 bytes, int32_t) - Số vòng quay hoàn chỉnh
    int32_t carry = 0;
    uint8_t carryData[4];
    for (int i = 0; i < 4; i++) {
      carryData[i] = Serial2.read();
      Serial.print("Carry Byte ");
      Serial.print(i);
      Serial.print(": 0x");
      Serial.println(carryData[i], HEX);
      carry |= (int32_t)carryData[i] << (24 - i * 8);
    }
    
    // Đọc value (2 bytes, uint16_t) - Góc trong vòng hiện tại
    uint16_t value = 0;
    uint8_t valueData[2];
    for (int i = 0; i < 2; i++) {
      ...
      Serial.print(i);
      Serial.print(": 0x");
      Serial.println(valueData[i], HEX);
    ...
    // TÍNH TOÁN ĐÚNG:
    // carry: số vòng quay hoàn chỉnh (revolution count)
    // value: góc trong vòng hiện tại (0-65535 tương ứng 0-360°)
    ...
    Serial.println("=== ENCODER CALCULATION ===");
    Serial.print("Carry (revolutions): ");
    Serial.println(carry);
    Serial.print("Value (raw): ");
    Serial.print(value);
    Serial.print(" = ");
    Serial.print(currentAngleInRevolution, 2);
    Serial.println("° in current revolution");
    Serial.print("Total angle: ");
    Serial.print(totalAngle, 2);
    Serial.println("°");
    Serial.print("Display angle (0-360°): ");
    Serial.print(displayAngle, 2);
    Serial.println("°");
    ...
    Serial.println(" revs)");
  ...
  } else if (command == 0x39) { // Góc sai số (int16_t) - BIG ENDIAN
      Serial.println("Reading error angle data (2 bytes)...");
      uint16_t value = 0;
      uint8_t data[2];
      for (int i = 0; i < 2; i++) {
        data[i] = Serial2.read();
      ...
      // Chuyển đổi sang signed int16 và tính góc
    ...
    Serial.println("Clearing remaining buffer...");
    while (Serial2.available()) {
